...
from net import Generator11 as Generator
from net import Discriminator11 as Discriminator
...
def train_one(gen, dis, optimizer_gen, optimizer_dis, x_batch, y_batch, gpu_device):
    ...
    x = gen((z, y))
    y1 = dis((x, y))
    t = Variable(xp.zeros(batch_size).astype(np.int32))
    loss_gen = F.softmax_cross_entropy(y1, t)
    acc_gen = F.accuracy(y1, t)
    t1 = Variable(xp.ones(batch_size).astype(np.int32))
    loss_dis = F.softmax_cross_entropy(y1, t1)
    acc_dis = F.accuracy(y1, t1)
    # train discriminator
    y2 = dis((Variable(xp.asarray(x_batch)), y))
    t2 = Variable(xp.zeros(batch_size).astype(np.int32))
    loss_dis += F.softmax_cross_entropy(y2, t2)
    acc_train = F.accuracy(y2, t2)
    ...
    return (float(loss_gen.data), float(loss_dis.data), float(acc_gen.data), float(acc_dis.data), float(acc_train.data))
...
def train(gen, dis, optimizer_gen, optimizer_dis, epoch_num, gpu_device=None, out_image_dir=None):
    ...
    for epoch in xrange(1, epoch_num + 1):
        ...
        for i in xrange(0, x_size, BATCH_SIZE):
            ...
            loss_gen, loss_dis, acc_gen, acc_dis, acc_train = train_one(gen, dis, optimizer_gen, optimizer_dis, x_batch, y_batch, gpu_device)
            ...
            sum_acc_train += acc_train * BATCH_SIZE
            if i % image_save_interval == 0:
                print '{} {}'.format(sum_loss_gen / (i + BATCH_SIZE), sum_loss_dis / (i + BATCH_SIZE))
                print '{} {} {}'.format(sum_acc_gen / (i + BATCH_SIZE), sum_acc_dis / (i + BATCH_SIZE), sum_acc_train / (i + BATCH_SIZE))
        ...
        print('train data accuracy={}'.format(sum_acc_train / x_size))
