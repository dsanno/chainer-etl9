...
from net import Generator3 as Generator, Discriminator3 as Discriminator
...
def train_one(gen, dis, optimizer_gen, optimizer_dis, x_batch, y_batch, gpu_device):
    ...
    t0 = Variable(xp.zeros(batch_size).astype(np.int32))
    loss_dis = F.softmax_cross_entropy(y1, t0)
    acc_dis = F.softmax_cross_entropy(y1, t0)
    # train discriminator
    y2 = dis(Variable(xp.asarray(x_batch)))
    loss_dis += F.softmax_cross_entropy(y2, t)
    acc_train = F.accuracy(y2, t)
...
def train(gen, dis, optimizer_gen, optimizer_dis, epoch_num, gpu_device=None, out_image_dir=None):
    ...
    for epoch in xrange(1, epoch_num + 1):
        ...
        print('train accuracy={}'.format(sum_acc_train / x_size))
