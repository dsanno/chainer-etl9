...
from net import Generator2, Discriminator2
        ...
        for i in xrange(0, x_size, BATCH_SIZE):
            ...
            if i % image_save_interval == 0:
                ...
                if out_image_dir != None:
                    y_gen = Variable(xp.asarray(range(out_image_len)).astype(np.int32))
                    data_array = gen((z_out_image, y_gen), train=False).data
                    for j, data in enumerate(data_array):
                        image = Image.fromarray(((1 - cuda.to_cpu(data)) * 255.99).astype(np.uint8).reshape(data.shape[1:3]))
                        image.save('{0}/{1:03d}_{2:03d}_{3:03d}.png'.format(out_image_dir, epoch, i, j))
