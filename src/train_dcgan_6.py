...
from net import Generator8 as Generator, Discriminator8 as Discriminator

parser = argparse.ArgumentParser(description='ETL9 DCGUN')
...
parser.add_argument('--dataset', '-d', default='dataset/etl9g64.pkl', type=str,
...
gen_model = Generator(LATENT_SIZE)
...
def train(gen, dis, optimizer_gen, optimizer_dis, epoch_num, gpu_device=None, out_image_dir=None):
    ...
    x_batch = np.zeros((BATCH_SIZE, 1, 64, 64), dtype=np.float32)
        ...
        for i in xrange(0, x_size, BATCH_SIZE):
            ...
            for j, p in enumerate(perm[i:i + BATCH_SIZE]):
                ...
                with io.BytesIO(image) as b:
                    pixels = np.asarray(Image.open(b).convert('L')).astype(np.float32).reshape((1, 64, 64))
                    x_batch[j, :, offset_y:offset_y + 61, offset_x:offset_x + 61] = pixels[:,3:64,2:63] / 255
            loss_gen, loss_dis = train_one(gen, dis, optimizer_gen, optimizer_dis, x_batch, gpu_device)
...
#            loss_dis = train_dis(gen, dis, optimizer_gen, optimizer_dis, x_batch, gpu_device)
            ...
            if i % image_save_interval == 0:
                ...
                if out_image_dir != None:
                    ...
                    image = ((1 - cuda.to_cpu(data)) * 256).clip(0, 255).astype(np.uint8).reshape((10, 10, 64, 64)).transpose((0, 2, 1, 3)).reshape((10 * 64, 10 * 64))
